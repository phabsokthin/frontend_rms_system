    ...
    <div class="min-h-screen">
        <div class="px-4">
            ...
            <div class="mb-[15px] flex items-center justify-between">
                <h1 class="text-xl font-bayon">បញ្ជីអតិថិជន</h1>
                <div class="flex gap-2">
                    <TextFieldInput v-model="searchTerm" placeholder="ស្វែងរកអតិថិជន" class="font-bayon" />
                ...
                <table class="min-w-[1300px] w-full border-collapse">
                    ...
                    <thead>
                        <tr class="text-white bg-green-700 text-md font-bayon">
                            ...
                            <th class="px-6 py-3 border">ចំណាំ</th>
...
export default {
    components: { Button, Pagination, Loading, DeletePopup, TextFieldInput,CustomerForm },
    setup() {
        ...
        onMounted(async () => {
            ...
            try {
                ...
                currentData.value = customer.getCustomer;
            } catch (err) {
                console.log(err);
            }
            finally {
        ...
        const filteredData = computed(() => {
            ...
            return currentData.value.filter(
                (item) =>
                    ...
                    (item.last_name && item.address.toLowerCase().includes(term))
            );
        })
        // Paginated data
        ...
        const handleCreatePopup = () => {
            currentComponent.value = "CustomerForm"
            updateData.value = ""
        }
        ...
        const handleDelete = async () => {
            try {
                ...
                const res = await customer.deleteData(setId.value)
                //real-time 
                await customer.fetchDta();
                currentData.value = customer.getCustomer;
                notify({
