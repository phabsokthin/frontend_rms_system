        ...
        <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            <div v-for="(order, index) in filteredData" :key="order._id || index" :class="[
                ...
                <div class="flex gap-2 mt-4">
                    ...
                    <!-- Always show Done -->
                    <button
                        class="flex-1 py-2 text-sm font-semibold text-white transition bg-blue-500 hover:bg-blue-600 font-bayon"
                        @click="handleToPaid(order._id)">
                        ទូទាត់
                    </button>
                </div>
            </div>
...
export default {
    ...
    setup() {
        ...
        // handle done
        const handleToPaid = async (id: string) => {
            try {
                if (window.confirm("Are you sure update status to paid?")) {
                    const data = {
                        _id: id,
                    }
