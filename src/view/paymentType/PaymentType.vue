    ...
    <div class="min-h-screen">
        <div class="px-4">
            ...
            <div class="mb-[15px] flex items-center justify-between">
                <h1 class="text-xl font-bayon">វីធីសាស្រ្តទូទាត់</h1>
                <div class="flex gap-2">
                    <TextFieldInput v-model="searchTerm" placeholder="ស្វែងរក" class="font-bayon" />
...
export default {
    components: { Button, Pagination, Loading, DeletePopup, TextFieldInput,PaymentTypeForm },
    setup() {
        const paymentType = paymentTypeStore()
        const currentData = ref<PaymentType[]>([]);
        ...
        onMounted(async () => {
            ...
            try {
                await paymentType.fetchDta();
                currentData.value = paymentType.getPaymentType;
            } catch (err) {
                console.log(err);
            }
            finally {
        ...
        const handleCreatePopup = () => {
            currentComponent.value = "PaymentTypeForm"
            updateData.value = ""
        }
        ...
        const handleDelete = async () => {
            try {
                const res = await paymentType.deleteData(setId.value)
